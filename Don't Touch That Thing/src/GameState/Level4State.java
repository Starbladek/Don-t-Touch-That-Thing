...
public class Level4State extends GameState {
	...
	private int powerUpSpawnTimerLength = 13000;
	
	private long scoreBallSpawnTimer;
	private long scoreBallElapsed;
	private int scoreBallSpawnTimerLength = 700;
	
	private int timeToLeaveAt = 120;
	...
	public Level4State(GameStateManager gsm) {
		...
		try {
			...
			bg = new Background(Content.Level4BG);
			bg.setVelocity(0, 0);
			bg.setPosition(0, 0);
	...
	public void init() {
		...
		thing = new Thing(players, scoreBalls, 0.85);
	...
	public void update() {
		...
		if (GameStateManager.timeLeft < timeToLeaveAt)
			gsm.wipeTransitionState(GameStateManager.LEVEL5STATE);
