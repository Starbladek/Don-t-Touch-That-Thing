...
public class Level5State extends GameState {
	...
	private int powerUpSpawnTimerLength = 12000;
	
	private long scoreBallSpawnTimer;
	private long scoreBallElapsed;
	private int scoreBallSpawnTimerLength = 600;
	
	private int timeToLeaveAt = 60;
	
	private int fadeOutVolume;
	private int fadeOutCurrentTime;
	...
	public Level5State(GameStateManager gsm) {
		...
		try {
			...
			bg = new Background(Content.Level5BG);
			bg.setVelocity(0, 0);
			bg.setPosition(0, 0);
	...
	public void init() {
		...
		thing = new Thing(players, scoreBalls, 0.9);
	...
	public void update() {
		...
		if (GameStateManager.timeLeft < timeToLeaveAt)
			gsm.wipeTransitionState(GameStateManager.LEVEL6STATE);
		...
		//Fade music out for finale
		if (GameStateManager.timeLeft < (timeToLeaveAt + 5)) {
			fadeOutCurrentTime++;
			fadeOutVolume = (int) Math.ceil(Utilities.Tweens.linearEase(fadeOutCurrentTime, -10, -50, fadeOutDuration));
			JukeBox.setVolume("GameMusic", fadeOutVolume);
			if (fadeOutCurrentTime > fadeOutDuration)
				fadeOutCurrentTime = fadeOutDuration;
		}
